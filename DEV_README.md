...
| `"discard_trash"`  | 쓰레기 폐기 | 쓰레기통으로 이동 → 손님이 떠난 테이블의 음식 폐기 |
...
| `entrance`           | object   | 입구 위치        | 손님 스폰 시작점 (걸어서 테이블로 이동) |
| `purchasable_tables` | object[] | 구매 가능 테이블 | 업그레이드로 활성화 가능한 테이블       |
| `trash_cans`         | object[] | 쓰레기통 위치    | 쓰레기통 그리드 좌표                    |
...
- **원점**: 좌상단 (0, 0)
- **X축**: 오른쪽으로 증가, range: 0 ~ 15 (width=16)
- **Y축**: 아래쪽으로 증가, range: 0 ~ 9 (height=10)
- **맵 구조**:
  ```
  Y=0  [벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽]
  Y=1  [벽 .  K  K  K  .  .  .  .  .  B  B  .  쓰 .  벽]  ← 주방(K)+바(B)+쓰레기(쓰)
  Y=2  [벽 .  .  .  .  .  .  .  .  .  .  .  .  .  .  벽]
  Y=3  [벽 .  T  .  T  .  T  .  T  .  .  .  .  .  .  벽]  ← 초기 테이블 4개
  Y=4  [벽 .  .  .  .  .  .  .  .  .  .  .  .  .  .  벽]
  Y=5  [벽 .  .  .  .  .  .  .  P  .  .  .  .  .  .  벽]  ← 플레이어 시작
  Y=6  [벽 .  (T) . (T) . (T) . (T) .  .  .  .  .  .  벽]  ← 구매 가능 테이블 4개
  Y=7  [벽 .  .  .  .  .  .  .  .  .  .  .  .  .  .  벽]
  Y=8  [벽 .  .  .  .  .  .  .  E  .  .  .  .  .  .  벽]  ← 입구(E): 손님 스폰
  Y=9  [벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽 벽]
...
- **화면 크기**: 1024 × 640 (게임) + 120 (UI) = 1024 × 760 (총)
- **Versus**: 좌우 분할 → 각 1024×760, 중앙 4px 구분선
...
| `_interact_trash()`      | 쓰레기통 상호작용 | 들고 있는 아이템 폐기                  |

### 시스템

| 메서드                                      | 한글             | 설명                                          |
| ------------------------------------------- | ---------------- | --------------------------------------------- |
| `_try_spawn_customer()`                     | 손님 배치        | 빈 테이블에 랜덤 손님 생성 (입구→테이블 이동) |
