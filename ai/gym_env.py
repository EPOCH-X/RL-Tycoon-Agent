...
from config.settings import TILE_SIZE, UI_HEIGHT, COLORS, NUM_ACTIONS, load_json_config
...
def _obs_size(shop: Shop) -> int:
    return 4 + 4 + 6 + shop.max_tables * 6 + 8 + 25 + 10 + 5
...
def build_observation(shop: Shop) -> np.ndarray:
    ...
    if first:
        ...
        if target_table is not None:
            obs[idx + 2] = np.clip((target_table.center_x - shop.player.center_x) / map_px_w, -1.0, 1.0)
            obs[idx + 3] = np.clip((target_table.center_y - shop.player.center_y) / map_px_h, -1.0, 1.0)
    idx += 4
    ...
    if priority_table is not None:
        ...
        obs[idx + 4] = np.clip((priority_table.center_x - shop.player.center_x) / map_px_w, -1.0, 1.0)
        obs[idx + 5] = np.clip((priority_table.center_y - shop.player.center_y) / map_px_h, -1.0, 1.0)
    idx += 6
        ...
        if i < len(shop.tables):
            ...
            if cust is not None:
                ...
                if mi:
                    obs[idx + 2] = MENU_IDS.get(mi["id"], 0) / NUM_MENU
                obs[idx + 3] = cust.patience_ratio
            obs[idx + 4] = np.clip((table.center_x - shop.player.center_x) / map_px_w, -1.0, 1.0)
            obs[idx + 5] = np.clip((table.center_y - shop.player.center_y) / map_px_h, -1.0, 1.0)
        idx += 6
    ...
    obs[idx + 14] = shop.waiting_queue[0].patience_ratio if queue_len > 0 else 0.0
    ...
    for offset, upgrade_id in enumerate(tracked_upgrades):
        ...
        obs[idx + 20 + offset] = 0.0 if cost is None else min(1.0, cost / max(1, shop.target_money))
    idx += 25
